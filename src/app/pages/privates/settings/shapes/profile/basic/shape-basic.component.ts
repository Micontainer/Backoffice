...
@Component({
  selector: 'app-shape-basic',
  templateUrl: './shape-basic.component.html',
  styleUrls: ['./shape-basic.component.scss']
})
export class ShapeBasicComponent implements OnInit {
  ...
  ngOnInit(): void {
    (async () => {
      ...
      const spaceRelations = await this.commonService.fetchRelations(this.shapeUID);
      shapeDTO.image = spaceRelations.find((element) => element.pk === 'ATTACHMENT');
      ...
      this.imageControl.setValue(shapeDTO.image);
  ...
  submitEventHandler(row: ResourceDTO): void {
    (async () => {
      try {
        row.attachmentUID = this.imageControl.value.uid;
        const shapeUID = await this.shapeService.save(row, this.shapeUID);

        let message = this.isEdition ? 'Se actualizó el objecto.' : 'Se creó el objecto.';
